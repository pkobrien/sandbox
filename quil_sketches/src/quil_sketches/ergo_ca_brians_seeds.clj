(ns quil-sketches.ergo-ca-brians-seeds
...
; ------------------------------------------------------------------------------
; Brian's Seeds Cellular Automata

(defn cell-state-candidate-set
  "Returns a function that determines the fate of a cell."
  [on-cell off-cell birth? on-count-f neighbors-f candidate?]
      ...
      (if-not (candidate? @index)
        ...
        (cond
          ...
          (= cell off-cell) (let [neighbors (neighbors-f @index)
                                  on-count (reduce on-count-f 0 neighbors)]
                              (if (birth? on-count) on-cell off-cell)))))))

(defn dense-candidate-set-ca-system
  [birth? neighborhood-f on-cell off-cell seed w h]
  (let [neighbors-lookup (ergo/make-neighborhood-lookup neighborhood-f w h)
        on-count-f (ergo/cell-counter on-cell)
        life (partial cell-state-candidate-set on-cell off-cell
                      birth? on-count-f)]
...
(defn setup [w setup-opts state]
  ...
  (let [on-color (q/color 300 100 100)
    ...
    (merge state
           {:draw-cell (get-draw-cell-f w on-color off-color)})))
...
(defn do-seeds [^long w ^long h fps]
  (let [system (dense-candidate-set-ca-system
                 #{2} ergo/neighborhood-8 :on :off
